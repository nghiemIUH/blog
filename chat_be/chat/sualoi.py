...
def predict(text, tokenize_ipt, tokenize_opt, transformer, maxlen=100,):
    ...
    encoder_input = tf.convert_to_tensor([encode])
    output = tf.convert_to_tensor([tokenize_opt.vocab_size], dtype=tf.int64)
    ...
    for i in range(maxlen):
        ...
        if predicted_id == tokenize_opt.vocab_size+1:
            break
        output = tf.concat([output, predicted_id], axis=-1)
    result = tokenize_opt.decode(
        [i for i in output.numpy()[0] if i < tokenize_opt.vocab_size])
    return result
